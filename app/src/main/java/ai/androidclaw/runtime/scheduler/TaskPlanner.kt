...
class TaskPlanner(
    ...
    fun initialScheduleAt(task: Task, now: Instant): Instant? {
        return task.nextRunAt ?: nextScheduledRun(task, now)
    }

    fun nextScheduledRun(task: Task, after: Instant): Instant? {
        return NextRunCalculator.computeNextRun(task.schedule, after)
    }

    fun nextRetryAt(task: Task, failureCount: Int, after: Instant): Instant? {
    ...
    fun selectNextRun(nextScheduledAt: Instant?, retryAt: Instant?): Instant? {
        return when {
