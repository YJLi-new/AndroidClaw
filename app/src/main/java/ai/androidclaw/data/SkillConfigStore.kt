...
interface SkillConfigStore {
    suspend fun readConfig(skillKey: String, configPath: String): String?

    suspend fun readConfigs(skillKey: String): Map<String, String>

    suspend fun writeConfig(skillKey: String, configPath: String, value: String?)
}
...
class AndroidSkillConfigStore(
    ...
    override suspend fun readConfig(skillKey: String, configPath: String): String? {
        return readPreferences()[storageKey(skillKey, configPath)]?.trim()?.takeIf(String::isNotBlank)
    }

    override suspend fun readConfigs(skillKey: String): Map<String, String> {
        val prefix = storagePrefix(skillKey)
        return readPreferences().asMap().entries
    ...
    private suspend fun readPreferences(): Preferences {
        return context.skillConfigDataStore.data.first()
    }

    private fun storageKey(skillKey: String, configPath: String) =
        stringPreferencesKey(storagePrefix(skillKey) + encodeSegment(configPath))

    private fun storagePrefix(skillKey: String): String {
        return "cfg:${encodeSegment(skillKey)}:"
    }
