...
class TaskExecutionWorkerTest {
    ...
    @Test
    fun `scheduled once task creates durable run and main-session output`() =
        runTest {
            ...
            val messages =
                application.container.messageRepository
                    .observeMessages(mainSession.id)
                    .first()
    ...
    @Test
    fun `manual run keeps future schedule intact`() =
        runTest {
            val mainSession = application.container.sessionRepository.getOrCreateMainSession()
            val task =
                application.container.taskRepository.createTask(
    ...
    @Test
    fun `scheduled interval task reschedules itself after success`() =
        runTest {
            val mainSession = application.container.sessionRepository.getOrCreateMainSession()
            val createdTask =
                application.container.taskRepository.createTask(
    ...
    @Test
    fun `scheduled cron task reschedules itself after success`() =
        runTest {
            val mainSession = application.container.sessionRepository.getOrCreateMainSession()
            val createdTask =
                application.container.taskRepository.createTask(
    ...
    @Test
    fun `isolated execution mode delivers summary to target session`() =
        runTest {
            val mainSession = application.container.sessionRepository.getOrCreateMainSession()
            val task =
                application.container.taskRepository.createTask(
    ...
    private fun buildWorker(
        ...
        TestListenableWorkerBuilder<TaskExecutionWorker>(application)
            .setWorkerFactory(application.container.workerFactory)
            .setInputData(
                Data
                    .Builder()
