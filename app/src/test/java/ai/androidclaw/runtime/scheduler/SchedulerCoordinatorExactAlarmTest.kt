...
class SchedulerCoordinatorExactAlarmTest {
    ...
    @Test
    fun `precise task uses exact alarm when permission is granted`() =
        runTest {
    ...
    @Test
    fun `precise task falls back to WorkManager when exact permission is denied`() =
        runTest {
            val alarmManager = requireNotNull(application.getSystemService(android.app.AlarmManager::class.java))
            val shadowAlarmManager = shadowOf(alarmManager)
...
private fun setCanScheduleExactAlarms(
    shadowAlarmManager: Any,
    value: Boolean,
) {
    val method =
        shadowAlarmManager.javaClass.getDeclaredMethod(
