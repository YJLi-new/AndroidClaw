...
class TaskRuntimeExecutorTest {
    ...
    @Test
    fun `isolated execution reports explicit failure when delivery write fails`() =
        runTest {
            val targetSession = sessionRepository.createSession("Delivery target")
            val throwingMessages =
                MessageRepository(
                    ThrowingMessageDao(
...
private class ThrowingMessageDao(
    ...
    override suspend fun getBeforeMessage(
        ...
        limit: Int,
    ): List<MessageEntity> = delegate.getBeforeMessage(sessionId, anchorMessageId, limit)

    override suspend fun getAfterMessage(
        sessionId: String,
        anchorMessageId: String,
        limit: Int,
    ...
    override suspend fun updateContentById(
        id: String,
        content: String,
    ): Int = delegate.updateContentById(id, content)
