...
class ExactAlarmRegressionTest {
    ...
    @Test
    fun preciseTaskMatchesExpectedExactAlarmPermissionState() = runBlocking {
        assumeTrue(android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S)
        val args = InstrumentationRegistry.getArguments()
        val expectedGranted = args.getString("expectedExactAlarmGranted")
            ?.toBooleanStrictOrNull()
            ?: error("expectedExactAlarmGranted instrumentation arg is required.")
        val verifyShellAlarm = args.getString("verifyShellAlarm")
            ?.toBooleanStrictOrNull()
            ?: false
