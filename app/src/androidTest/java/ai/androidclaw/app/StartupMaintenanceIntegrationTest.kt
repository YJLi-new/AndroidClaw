...
class StartupMaintenanceIntegrationTest {
    ...
    @Test
    fun startupMaintenance_recreatesMainSession_prunesHistory_andReschedulesPendingWork() =
        runBlocking {
            val now = Instant.now()
            ...
            assertEquals(
                ...
                application.container.database
                    .messageDao()
                    .countBySessionId(mainSession!!.id),
            )
            assertEquals(
                "AndroidClaw is ready.",
                application.container.messageRepository
                    .getRecentMessages(mainSession.id, limit = 1)
                    .single()
                    .content,
            )
            assertNotNull(
