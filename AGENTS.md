...
1. Read `PLAN.md` before making any non-trivial change.
2. Treat `PLAN.md` as the current source of truth for architecture, milestones, acceptance criteria, and trade-offs.
3. If the task changes scope or invalidates the plan, update `PLAN.md` first, then code.
4. Keep `PLAN.md` current while you work. It is a living document, not a static design note.
...
6. Update `PLAN.md` with progress, discoveries, and decisions.
...
These are non-negotiable for v0 unless `PLAN.md` is explicitly changed.
...
These are out of scope unless `PLAN.md` explicitly adds them:
...
- Put active implementation detail in `PLAN.md`.
- If work becomes too large for one plan, create a child plan under `docs/exec-plans/active/<slug>.md` and link it from `PLAN.md`.
...
5. Record the trade-off in `PLAN.md`.
