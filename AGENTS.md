...
1. Read `PLANv6.md` before making any non-trivial change.
2. Treat `PLANv6.md` as the current source of truth for architecture, milestones, acceptance criteria, and trade-offs.
3. If the task changes scope or invalidates the plan, update `PLANv6.md` first, then code.
4. Keep `PLANv6.md` current while you work. It is a living document, not a static design note.
...
6. Update `PLANv6.md` with progress, discoveries, and decisions.
...
These are non-negotiable for v0 unless `PLANv6.md` is explicitly changed.
...
These are out of scope unless `PLANv6.md` explicitly adds them:
...
- `AGENTS.md`
- `PLANv6.md`
- archived history: `PLANv1.md`, `PLANv2.md`, `PLANv3.md`, `PLANv4.md`, `PLANv5.md`
- `app/`
...
- Put active implementation detail in `PLANv6.md`.
- If work becomes too large for one plan, create a child plan under `docs/exec-plans/active/<slug>.md` and link it from `PLANv6.md`.
...
5. Record the trade-off in `PLANv6.md`.
